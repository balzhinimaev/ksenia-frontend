        ...
        <form v-if="settings" @submit.prevent="updateSettings" class="mt-8 bg-white p-8 rounded-lg shadow space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input type="number" v-model.number="settings.currentPrice" id="currentPrice" class="mt-1 block w-full input-field" />
            </div>
            <div>
              <label for="basePrice" class="block text-sm font-medium text-gray-700">Базовая цена</label>
              <input type="number" v-model.number="settings.basePrice" id="basePrice" class="mt-1 block w-full input-field" />
            </div>
            <div>
              <label for="cardNumber" class="block text-sm font-medium text-gray-700">Номер карты</label>
              <input type="text" v-model="settings.cardNumber" id="cardNumber" class="mt-1 block w-full input-field" />
            </div>
            <div>
              <label for="cardHolderName" class="block text-sm font-medium text-gray-700">Имя держателя карты</label>
              <input type="text" v-model="settings.cardHolderName" id="cardHolderName" class="mt-1 block w-full input-field" />
            </div>
          </div>
          <div class="space-y-6">
            <div>
              <label for="otherCountries" class="block text-sm font-medium text-gray-700">Ссылка для других стран</label>
              <input type="url" v-model="settings.otherCountries" id="otherCountries" class="mt-1 block w-full input-field" />
            </div>
            <div>
              <label for="sendTo" class="block text-sm font-medium text-gray-700">Отправлять на (email/id)</label>
              <input type="text" v-model="settings.sendTo" id="sendTo" class="mt-1 block w-full input-field" />
            </div>
          </div>
...
<style scoped>
.input-field {
  @apply px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm;
}
.btn-primary {
